...
- Пройти базовые задачи на LeetCode Easy или [Go Playground](https://go.dev/play/).
    
- Потренироваться объяснять код **вслух** (это ключевой навык).
    
- Освежить:
    
    - структуры данных (`array`, `map`, `queue`, `stack`),
        
    - обход деревьев (`BFS`, `DFS`),
        
    - сортировки (`bubble`, `quick`, `merge`),
        
    - поиск (`linear`, `binary`),
        
    - динамику (лестница, Фибоначчи).