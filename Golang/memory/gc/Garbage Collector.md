...
## Способы вызова **сборщика мусора (GC)**:

1. При достижении лимита `{go} GOGC`; 

   Как это работает : 
   
    `{go} GC` запускается автоматически при достижении соотношения
     `{go} freshly allocated memory` / `{go} live memory` -> `{go} GOGC / 100`;
     
     По умолчанию `{go} GOGC` = 100 (100%);
     
     Пример: При 10 мб живой памяти, `{go} GC` запустится когда новая память достигнет ~10 МБ
     
     Цель: Контроль затрат на `{go} GC` (~25% от общего времени CPU);
---
2. По таймеру / Планировщику : 

	Как это работает : 
	  
	  Фоновая горутина запускает `{go} GC` примерно каждые 2 минуты 
	  или при изменениях размера кучи.
	
	Когда : Go 1.15+
	
	Цель: Распределение нагрузки GC, избежание больших STW-пауз.
---
3. Принудительный вызов (`{go} runtime.GC()`) : 
```go
import "runtime"
...
Предупреждение: Избегать в production-коде

Использование: Тестирование, отладка, бенчмарки

---
4. При нехватке памяти (аварийный) : 

   Как работает: 
	Системный монитор (`{go} sysmon`) форсирует `{go} GC` 
	при получении сигналов от ОС о нехватке памяти;
	
   Триггер: 
    Сильная нехватка RAM, 
    приближение к лимитам `{go} cgroups` (Docker);
